...
export class CompositeLogger implements LoggerService {
    constructor(
        private readonly delegates: LoggerService[]
    ) { }
    ...
    debug?(message: any, ...optionalParams: any[]) {
        for (const d of this.delegates) { if (d.debug) d.debug(message, ...optionalParams); }
    }

    verbose?(message: any, ...optionalParams: any[]) {
        for (const d of this.delegates) { if (d.verbose) d.verbose(message, ...optionalParams); }
    }
